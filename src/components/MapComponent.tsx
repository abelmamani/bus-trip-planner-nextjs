...
function AutoCenter({ origin, destination }: any) {
  const map = useMap();
...
export default function MapComponent() {
  ...
  const transfer = useStopStore((state) => state.transfer);
  ...
  return (
    <div style={{ height: "100vh", width: "100%" }}>
      ...
      >
        ...
        <AutoCenter origin={origin} destination={destination} />
        {origin && (
          <Marker
            position={[origin.latitude, origin.longitude]}
            icon={DefaultIcon}
          >
            <Popup>{"Parada Origen: " + origin.name}</Popup>
          </Marker>
        )}
        {transfer && (
          <Marker
            position={[transfer.latitude, transfer.longitude]}
            icon={DefaultIcon}
          >
            <Popup>{"Parada Trasbordo " + transfer.name}</Popup>
          </Marker>
        )}
        {destination && (
          <Marker
            ...
            icon={DefaultIcon}
          >
            <Popup>{"Parada Destino: " + destination.name}</Popup>
          </Marker>
        )}
        {fromShape && (
          <Polyline
            positions={fromShape.map((shape) => [
              shape.latitude,
              shape.longitude,
            ])}
            color="blue"
